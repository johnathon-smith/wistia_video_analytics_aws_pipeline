"""Build the refined dim_media Delta table from validated Wistia events.
...
Optional arguments:
    ...
    --REFINED_PREFIX          Default: refined/dim_media
    --DIM_MEDIA_TABLE_URI     Overrides the inferred Delta table URI
...
class DimMediaError(RuntimeError):
    """Raised when dim_media cannot be built safely."""
...
def parse_optional_argument(name: str, default: str | None = None) -> str | None:
    ...
    if index + 1 >= len(sys.argv) or sys.argv[index + 1].startswith("--"):
        raise DimMediaError(f"{flag} requires a value.")
...
def load_config() -> JobConfig:
    ...
    return JobConfig(
        ...
        dim_media_table_uri=parse_optional_argument("DIM_MEDIA_TABLE_URI"),
...
def workflow_context(config: JobConfig) -> tuple[str, str] | None:
    ...
    if not config.workflow_name or not config.workflow_run_id:
        raise DimMediaError("WORKFLOW_NAME and WORKFLOW_RUN_ID must both be supplied.")
...
def read_validation_report(s3_client: Any, uri: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise DimMediaError(f"Unable to read validation report {uri}.") from exc
    if not isinstance(report, dict):
        raise DimMediaError(f"Expected a JSON object at {uri}.")
    return report
...
def resolve_raw_input(
    ...
    if not isinstance(raw_uri, str):
        raise DimMediaError("Validation report is missing raw_s3_uri.")
...
def channel_from_title(title: str | None) -> str | None:
    if not isinstance(title, str):
        return None
    lowered = title.lower()
    has_youtube = "youtube" in lowered
    has_facebook = "facebook" in lowered
    if has_youtube == has_facebook:
        return None
    return "Youtube" if has_youtube else "Facebook"
...
def build_dimension_dataframe(spark: Any, raw_input_uri: str) -> Any:
    ...
    required_columns = {"media_id", "media_name", "media_url", "received_at"}
    ...
    channel_udf = functions.udf(channel_from_title, StringType())
    candidates = (
        events.select(
            functions.col("media_id"),
            functions.col("media_name").alias("title"),
            functions.col("media_url").alias("url"),
            functions.to_timestamp("received_at").alias("_received_at"),
        )
        .withColumn("channel", channel_udf(functions.col("title")))
    )

    invalid_channels = candidates.filter(functions.col("channel").isNull()).select(
        "media_id", "title"
    )
    invalid_examples = invalid_channels.limit(10).collect()
    if invalid_examples:
        examples = ", ".join(
            f"{row['media_id']}={row['title']!r}" for row in invalid_examples
        )
        raise DimMediaError(
    ...
    latest_per_media = Window.partitionBy("media_id").orderBy(
        ...
        functions.col("url").desc(),
    )
    return (
        candidates.withColumn("_row_number", functions.row_number().over(latest_per_media))
        .filter(functions.col("_row_number") == 1)
        .select("media_id", "title", "url", "channel")
    )
...
def upsert_delta_table(spark: Any, dimension: Any, table_uri: str) -> int:
    ...
    if row_count == 0:
        LOGGER.info("No media records were found; leaving the Delta table unchanged.")
        return 0
    ...
    if DeltaTable.isDeltaTable(spark, table_uri):
        ...
        dimension.write.format("delta").mode("overwrite").save(table_uri)
    return row_count
...
def publish_workflow_properties(
    ...
    try:
        glue_client.put_workflow_run_properties(
            ...
            RunId=workflow_run_id,
            RunProperties={
                "DIM_MEDIA_TABLE_URI": table_uri,
                "DIM_MEDIA_ROW_COUNT": str(row_count),
                "DIM_MEDIA_INGESTION_RUN_ID": run_input.ingestion_run_id,
            },
        )
    except (BotoCoreError, ClientError) as exc:
        raise DimMediaError(
            f"Unable to publish dim_media properties for workflow "
...
def main() -> None:
    ...
    LOGGER.info(
        "Building dim_media ingestion_run_id=%s raw_input_uri=%s table_uri=%s",
        run_input.ingestion_run_id,
        raw_input_uri,
        table_uri,
    )
    if input_record_count == 0:
        LOGGER.info("Validation report contains no valid records; leaving dim_media unchanged.")
        row_count = 0
    else:
        dimension = build_dimension_dataframe(spark, raw_input_uri)
        row_count = upsert_delta_table(spark, dimension, table_uri)
    ...
    LOGGER.info(
        "Completed dim_media upsert row_count=%s table_uri=%s ingestion_run_id=%s",
        row_count,
        table_uri,
        run_input.ingestion_run_id,
    )
...
if __name__ == "__main__":
    try:
        ...
        LOGGER.exception("dim_media build failed.")
        raise
