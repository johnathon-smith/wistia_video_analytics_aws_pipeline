"""Build the refined dim_visitors Delta table from validated Wistia events.
...
Optional arguments:
    ...
    --REFINED_PREFIX             Default: refined/dim_visitors
    --DIM_VISITORS_TABLE_URI     Overrides the inferred Delta table URI
...
class DimVisitorsError(RuntimeError):
    """Raised when dim_visitors cannot be built safely."""
...
class JobConfig:
    ...
    refined_prefix: str
    dim_visitors_table_uri: str | None
...
def parse_optional_argument(name: str, default: str | None = None) -> str | None:
    ...
    if index + 1 >= len(sys.argv) or sys.argv[index + 1].startswith("--"):
        raise DimVisitorsError(f"{flag} requires a value.")
...
def load_config() -> JobConfig:
    ...
    return JobConfig(
        ...
        dim_visitors_table_uri=parse_optional_argument("DIM_VISITORS_TABLE_URI"),
...
def workflow_context(config: JobConfig) -> tuple[str, str] | None:
    ...
    if not config.workflow_name or not config.workflow_run_id:
        raise DimVisitorsError("WORKFLOW_NAME and WORKFLOW_RUN_ID must both be supplied.")
...
def read_validation_report(s3_client: Any, uri: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise DimVisitorsError(f"Unable to read validation report {uri}.") from exc
    if not isinstance(report, dict):
        raise DimVisitorsError(f"Expected a JSON object at {uri}.")
    return report
...
def resolve_raw_input(
    ...
    if not isinstance(raw_uri, str):
        raise DimVisitorsError("Validation report is missing raw_s3_uri.")
...
def build_dimension_dataframe(spark: Any, raw_input_uri: str) -> Any:
    ...
    required_columns = {"visitor_key", "ip", "country", "received_at"}
    ...
    candidates = events.select(
        ...
        functions.col("ip").alias("ip_address"),
    ...
    latest_per_visitor = Window.partitionBy("visitor_id").orderBy(
        ...
        functions.col("country").desc(),
    )
    return (
        candidates.withColumn(
            "_row_number",
            functions.row_number().over(latest_per_visitor),
        )
        .filter(functions.col("_row_number") == 1)
        .select("visitor_id", "ip_address", "country")
    )
...
def upsert_delta_table(spark: Any, dimension: Any, table_uri: str) -> int:
    ...
    if row_count == 0:
        LOGGER.info("No visitor records were found; leaving the Delta table unchanged.")
        return 0
...
def publish_workflow_properties(
    ...
    if context is None:
        LOGGER.info(
            "No Glue workflow context found; skipping dim_visitors property publication."
        )
        return
    ...
    try:
        glue_client.put_workflow_run_properties(
            ...
            RunId=workflow_run_id,
            RunProperties={
                "DIM_VISITORS_TABLE_URI": table_uri,
                "DIM_VISITORS_ROW_COUNT": str(row_count),
                "DIM_VISITORS_INGESTION_RUN_ID": run_input.ingestion_run_id,
            },
        )
    except (BotoCoreError, ClientError) as exc:
        raise DimVisitorsError(
            f"Unable to publish dim_visitors properties for workflow "
...
def main() -> None:
    ...
    LOGGER.info(
        "Building dim_visitors ingestion_run_id=%s raw_input_uri=%s table_uri=%s",
    ...
    if input_record_count == 0:
        LOGGER.info(
            "Validation report contains no valid records; leaving dim_visitors unchanged."
        )
        row_count = 0
    else:
    ...
    LOGGER.info(
        "Completed dim_visitors upsert row_count=%s table_uri=%s ingestion_run_id=%s",
        row_count,
        table_uri,
        run_input.ingestion_run_id,
    )
...
if __name__ == "__main__":
    try:
        ...
        LOGGER.exception("dim_visitors build failed.")
        raise
